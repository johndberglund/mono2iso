...
// parsing it back out with a regex. Mirrors the same change in aniso.js -- keep both in sync.
var tileEdges;
// vertexAngle[i] from specify()'s reducibility check: 1 if position i is a genuine
// combinatorial vertex (some angle equation ties to it), 0 if it's a redundant/reducible
// position -- structurally determined by (id, m, orient, off, si) ALONE, independent of the
// solved numeric angle there. Exposed the same way as tileEdges (John, 2026-08-05: interior
// vertices of a reduced edge group are free to move, like curve control points -- comparing
// two tiles as shapes should look at the real corners only). Mirrors aniso.js.
var reducibleVertex;
// The final, sorted `tileAngles` matrix (each row: tileLength+1 numbers -- m angle
// coefficients + a constant term), exposed the same way as tileEdges/reducibleVertex so
// callers can read the angle-equation system directly instead of round-tripping through the
// "tileAngles" text block and re-parsing it with a regex (John, 2026-08-05: "let aniso.js talk
// to other programs" instead of only printing for a human to read). Rows are in the same order
// specify() would have printed them, AFTER the row-sorting passes below. Mirrors aniso.js.
var tileAnglesOut;
...
// go through all pairs that are mapped. change the higher one to the lower.
   ...
   for ( i = 0;i<tileLength;i++) {
    ...
    // ---- OPTIMISED (aniso_fast.js only; aniso.js keeps the original) ----------------------
    // Same fixed point, computed with a worklist instead of by restarting the scan.
    //
    // The rule is: within one row, if ANY nonzero column is already a vertex, then EVERY nonzero
    // column of that row becomes a vertex. The original found a new vertex and then reset
    // `i = netAngles.length; j = 0` to rescan everything from the top, so it paid
    // O(vertices x rows x tileLength). Because the rule is monotone (flags only ever go 0->1)
    // the fixed point does not depend on visit order, so a worklist reaches the identical
    // result in O(total nonzeros): each row can "fire" at most once.
    //
    // Measured at 28% of adjacentSetUp's self time (V8 line ticks, #69445 m=40) -- lines 650/654
    // of aniso.js were the two hottest lines in the whole config path.
    if (netAngles.length < angleCounter) {
      // rowsWithCol[c] = indices of candidate rows having a nonzero in column c
      var rowsWithCol = new Array(tileLength);
      for (var c0 = 0; c0 < tileLength; c0++) rowsWithCol[c0] = [];
      for (var r0 = netAngles.length; r0 < angleCounter; r0++) {
        var rowR = tileAngles[r0];
        for (var c1 = 0; c1 < tileLength; c1++) if (rowR[c1] != 0) rowsWithCol[c1].push(r0);
      }
      var fired = new Uint8Array(angleCounter);
      var work = [];
      for (var c2 = 0; c2 < tileLength; c2++) if (vertexAngle[c2] === 1) work.push(c2);
      while (work.length) {
        var col = work.pop();
        var rl = rowsWithCol[col];
        for (var t0 = 0; t0 < rl.length; t0++) {
          var rr = rl[t0];
          if (fired[rr]) continue;
          fired[rr] = 1;
          var rowV = tileAngles[rr];
          for (var c3 = 0; c3 < tileLength; c3++) {
            if (rowV[c3] != 0 && vertexAngle[c3] === 0) { vertexAngle[c3] = 1; work.push(c3); }
          }
        }
      }
    }
